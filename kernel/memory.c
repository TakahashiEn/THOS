...
void pagetable_init()
{
	...
	for(i = 0;i < memory_management_struct.zones_size;i++)
	{
		...
		for(j = 0;j < z->pages_length ; j++,p++)
		{
			...
			// 这块zone 内的Page 对应的顶层页表项的地址
			tmp = (unsigned long *) (
					((unsigned long)Phy_To_Virt((unsigned long)Global_CR3 & (~ 0xfffUL))) +\
					(((unsigned long)Phy_To_Virt(p->PHY_address) >> PAGE_GDT_SHIFT) & 0x1ff) * 8
				);
			
			// 顶层页表没有内容证明还没有还没有下级页表被创建， 所以要先申请内存创建下级页表
			if(*tmp == 0)
			{			
			...
			tmp = (unsigned long *)(
					(unsigned long)Phy_To_Virt(*tmp & (~ 0xfffUL)) + \
